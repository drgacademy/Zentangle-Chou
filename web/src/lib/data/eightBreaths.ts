/**
 * The 8 official Zentangle steps — Maria Thomas / Rick Roberts.
 *
 * We render each step as a numbered card, paired with a small illustration
 * cue (the layer of the tile being added at that step).
 */

export type BreathStep = {
  index: number;
  i18nKey: string; // e.g. 'eightBreaths.step1'
  layerHint: 'gratitude' | 'dot' | 'pencil-border' | 'pencil-string' | 'ink' | 'shade' | 'initial' | 'appreciate';
};
